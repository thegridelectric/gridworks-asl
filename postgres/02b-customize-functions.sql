...
RETURNS BOOLEAN AS $$
  SELECT (calc_enum_versions_is_active(enum_versions_id)::boolean) FROM enum_versions
   WHERE name = (SELECT enum_version_ref FROM type_attributes WHERE type_attributes_id = p_type_attributes_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS BOOLEAN AS $$
  SELECT (calc_type_versions_is_active(type_versions_id)::boolean) FROM type_versions
   WHERE name = (SELECT sub_type_version_ref FROM type_attributes WHERE type_attributes_id = p_type_attributes_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (calc_type_versions_owner_name(type_versions_id)::text) FROM type_versions
   WHERE name = (SELECT type_version FROM type_examples WHERE type_examples_id = p_type_examples_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (calc_type_versions_owner_name(type_versions_id)::text) FROM type_versions
   WHERE name = (SELECT type_version FROM type_axioms WHERE type_axioms_id = p_type_axioms_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (calc_type_versions_owner_name(type_versions_id)::text) FROM type_versions
   WHERE name = (SELECT origin_type_version FROM type_helpers WHERE type_helpers_id = p_type_helpers_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS BOOLEAN AS $$
  SELECT (calc_enum_versions_is_draft(enum_versions_id)::boolean) FROM enum_versions
   WHERE name = (SELECT enum_version_ref FROM type_helper_attributes WHERE type_helper_attributes_id = p_type_helper_attributes_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS BOOLEAN AS $$
  SELECT (calc_type_versions_is_draft(type_versions_id)::boolean) FROM type_versions
   WHERE name = (SELECT sub_type_version_ref FROM type_helper_attributes WHERE type_helper_attributes_id = p_type_helper_attributes_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (calc_enum_versions_owner_name(enum_versions_id)::text) FROM enum_versions
   WHERE name = (SELECT from_enum_version FROM projections WHERE projections_id = p_projections_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (calc_enum_versions_owner_name(enum_versions_id)::text) FROM enum_versions
   WHERE name = (SELECT to_enum_version FROM projections WHERE projections_id = p_projections_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (type::text) FROM type_versions
   WHERE name = (SELECT from_type_version FROM type_upgrades WHERE type_upgrades_id = p_type_upgrades_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (type::text) FROM type_versions
   WHERE name = (SELECT to_type_version FROM type_upgrades WHERE type_upgrades_id = p_type_upgrades_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (enum::text) FROM enum_versions
   WHERE name = (SELECT from_enum_version FROM enum_upgrades WHERE enum_upgrades_id = p_enum_upgrades_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS TEXT AS $$
  SELECT (enum::text) FROM enum_versions
   WHERE name = (SELECT to_enum_version FROM enum_upgrades WHERE enum_upgrades_id = p_enum_upgrades_id)
   LIMIT 1;
$$ LANGUAGE sql STABLE;
...
RETURNS INTEGER AS $$
  SELECT (COUNT(*))::integer
   ...
   WHERE t.type_version = (SELECT name FROM type_versions WHERE type_versions_id = p_type_versions_id) AND calc_type_attributes_ref_is_stale(t.type_attributes_id) = TRUE;
$$ LANGUAGE sql STABLE;
