...
-- ----------------------------------------------------------------------------
-- vw_cli_commands: The hierarchical surface of the `sema` CLI — every parser node from src/sema/interfaces/cli/. Each row is one subcommand or the root. Parent links form a tree; flags and examples attach via CliFlags.Command and CliExamples.Command. This is descriptive scaffolding (Tier B of ERB-SCAFFOLD-PLAN.md) — the CLI is still implemented in Python, but the rulebook now points at every node.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_cli_commands WITH (security_invoker = ON) AS
SELECT
  t.cli_commands_id,                                                            -- Primary key (auto-synthesized: CliCommands had no field named 'Id' or ending in 'Id').
  t.name,                                                                       -- Fully-qualified command path. Example: 'sema snapshot prepare'. Acts as the primary key.
  t.parent,                                                                     -- FK to the parent CliCommand. NULL for the root 'sema'. Children's Names start with their Parent's Name followed by a single space.
  t.impl_file,                                                                  -- Path (relative to repo root) of the Python file that defines this command's parser/handler.
  t.summary,                                                                    -- Short one-line description, mirroring the argparse `help=` for this subparser.
  t.description,                                                                -- Longer description, mirroring the argparse `description=` for this subparser. Often spans multiple sentences.
  calc_cli_commands_child_count(t.cli_commands_id) AS child_count,              -- Number of CliCommands whose Parent is this command.
  calc_cli_commands_flag_count(t.cli_commands_id) AS flag_count,                -- Number of CliFlags attached to this command.
  calc_cli_commands_example_count(t.cli_commands_id) AS example_count,          -- Number of CliExamples attached to this command.
  calc_cli_commands_is_leaf(t.cli_commands_id) AS is_leaf                       -- True when this command has no children (a runnable subcommand, not a parser-only grouping).
FROM cli_commands t;

-- ----------------------------------------------------------------------------
-- vw_cli_flags: Every flag (positional or named) declared on a CliCommand. Name is the compound key Command::FlagName. Kind names a primitive value type; EnumValuesRef points at an Enum when a flag's value is constrained to a rulebook enum (no current flag uses this, but the column is reserved for future use).
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_cli_flags WITH (security_invoker = ON) AS
SELECT
  t.cli_flags_id,                                                               -- Primary key (auto-synthesized: CliFlags had no field named 'Id' or ending in 'Id').
  calc_cli_flags_name(t.cli_flags_id) AS name,                                  -- Compound key: <Command>::<FlagName>. Auto-derived.
  t.command,                                                                    -- FK to the CliCommands row this flag attaches to.
  t.flag_name,                                                                  -- Flag spelling. Positional args use the bare identifier ('name', 'version'). Named args use the long form including dashes ('--package-name').
  t.short_name,                                                                 -- Short alias if defined ('-p'). NULL for positional args and named-only flags.
  t.is_positional,                                                              -- True when the argparse action is a positional argument. False for named flags.
  t.kind,                                                                       -- Primitive value type. One of 'string', 'bool', 'int', 'path'. Reserved value 'enum' applies when EnumValuesRef is non-null.
  t.required,                                                                   -- True when the flag is mandatory. For positional args this maps to nargs not being '?' or '*'.
  t."default",                                                                  -- Stringified default value, or NULL when there is no default.
  t.description,                                                                -- Short description, taken from argparse `help=` where available.
  t.enum_values_ref                                                             -- When the flag accepts values constrained to a rulebook Enum, points at the Enum row. NULL otherwise.
FROM cli_flags t;

-- ----------------------------------------------------------------------------
-- vw_cli_examples: Concrete example invocations for a CliCommand. Sourced from argparse `epilog=` blocks. Idx preserves listing order. ExpectedExitCode defaults to 0 (success); future rows can document expected-failure examples.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_cli_examples WITH (security_invoker = ON) AS
SELECT
  t.cli_examples_id,                                                            -- Primary key (auto-synthesized: CliExamples had no field named 'Id' or ending in 'Id').
  calc_cli_examples_name(t.cli_examples_id) AS name,                            -- Compound key: <Command>::ex<Idx>. Auto-derived.
  t.command,                                                                    -- FK to the CliCommands row this example illustrates.
  t.idx,                                                                        -- 0-indexed ordering of this example within its Command's epilog.
  t.example_string,                                                             -- The full command-line invocation as it would be typed in a terminal.
  t.description,                                                                -- Optional short note about what this example demonstrates.
  t.expected_exit_code                                                          -- POSIX exit code the example is expected to produce. 0 for success.
FROM cli_examples t;

-- ----------------------------------------------------------------------------
-- vw_yaml_files: Pointer rows for every YAML file under definitions/ that contributes to the rulebook (Tier C of ERB-SCAFFOLD-PLAN.md). Name is the file path relative to the repo root. Kind selects which (nullable) FK column is populated. IsGenerated is FALSE today since the YAMLs are still the SSoT; after Phase 3 of YAML-ODXML-RULEBOOK-MIGRATION-PLAN.md flips it to TRUE.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_yaml_files WITH (security_invoker = ON) AS
SELECT
  t.yaml_files_id,                                                              -- Primary key (auto-synthesized: YamlFiles had no field named 'Id' or ending in 'Id').
  t.name,                                                                       -- Path relative to the repo root. Example: 'definitions/types/atn.bid/002.yaml'. Acts as the primary key.
  t.kind,                                                                       -- One of 'type', 'enum', 'format', 'registry', 'owner'. Determines which FK column is populated.
  t.type_version,                                                               -- FK to TypeVersions when Kind='type' and the file lives at definitions/types/<name>/<version>.yaml.
  t.enum_version,                                                               -- FK to EnumVersions when Kind='enum'.
  t.format,                                                                     -- FK to Formats when Kind='format'.
  t.sha,                                                                        -- Optional SHA-256 hex digest of the file contents. NULL today; future tooling can populate.
  t.is_generated,                                                               -- FALSE while the YAML is the SSoT (current state). After Phase 3 of YAML-ODXML-RULEBOOK-MIGRATION-PLAN.md, the rulebook becomes the SSoT and these files are emitted by rulebook-to-yaml — set TRUE then.
  calc_yaml_files_has_rulebook_row(t.yaml_files_id) AS has_rulebook_row         -- TRUE iff at least one of TypeVersion / EnumVersion / Format is non-null. FALSE for registry/owner kinds and for any orphan YAML file that lacks a corresponding rulebook row.
FROM yaml_files t;

-- ----------------------------------------------------------------------------
-- vw_emitters: Catalog of rulebook emitters (Tier D of ERB-SCAFFOLD-PLAN.md). Every subdirectory of rulebook-emitters/ that consumes the rulebook and writes downstream artifacts is a row here. shared/ is not an emitter — it's a common-utilities library — and is excluded. Match the table at the top of rulebook-emitters/README.md.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_emitters WITH (security_invoker = ON) AS
SELECT
  t.emitters_id,                                                                -- Primary key (auto-synthesized: Emitters had no field named 'Id' or ending in 'Id').
  t.name,                                                                       -- Emitter slug — the subdirectory name under rulebook-emitters/. Acts as the primary key.
  t.path,                                                                       -- Path to the emitter's root directory, relative to the repo root.
  t.language,                                                                   -- Language or format of the generated artifacts. Today same as Name; reserved separately so future emitters can target a language under a non-matching dir name.
  t.input_kind,                                                                 -- What the emitter reads. Currently always 'rulebook'; reserved for future 'snapshot'-driven emitters that consume output/sema/ instead.
  t.output_dir,                                                                 -- Where this emitter writes its artifacts. yaml/ writes to definitions-emitted/ at the repo root; the others write under <emitter>/out/.
  t.entrypoint_module,                                                          -- Importable Python module path for the emitter's CLI entrypoint.
  t.has_templates,                                                              -- TRUE iff the emitter has a templates/ subdirectory of Jinja or stdlib-Template files.
  t.round_trips,                                                                -- TRUE iff the emitter is paired with an inverse importer that closes the loop (rulebook → emitted → rulebook). Only yaml/ today.
  t.round_trip_percent,                                                         -- Latest measured round-trip parity (0–100). NULL when RoundTrips is FALSE.
  t.maturity,                                                                   -- One of 'real' (in production use; generated artifacts committed and consumed) or 'scaffold' (entrypoint and templates exist, not wired into the build). Matches the Status column of rulebook-emitters/README.md, renamed to avoid collision with versioned-protocol Status semantics.
  t.description                                                                 -- Free-form notes about what this emitter produces and any caveats.
FROM emitters t;

-- ----------------------------------------------------------------------------
-- vw_features: Coarse-grained capabilities that the sema platform delivers (Tier E of ERB-SCAFFOLD-PLAN.md). Each row is a human-readable index into 1..N implementing pieces — CliCommands, Emitters, IndexBuilders, Templates, and tables — surfaced via FeatureBindings. Whether a Feature is shipped or merely planned is derived from BindingCount; there is no Status field.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_features WITH (security_invoker = ON) AS
SELECT
  t.features_id,                                                                -- Primary key (auto-synthesized: Features had no field named 'Id' or ending in 'Id').
  t.name,                                                                       -- Short snake-case feature name. Acts as the primary key.
  t.summary,                                                                    -- One-line description of the capability.
  t.description,                                                                -- Longer description of what the feature does and how it's experienced by users.
  t.owner,                                                                      -- FK to Owners. Who maintains this feature.
  t.introduced_in_commit,                                                       -- Optional pointer to the commit sha that first introduced this capability. NULL when historical attribution is unclear.
  calc_features_binding_count(t.features_id) AS binding_count,                  -- Total number of FeatureBindings rows pointing at this feature.
  calc_features_cli_command_count(t.features_id) AS cli_command_count,          -- Number of CLI commands bound to this feature.
  calc_features_emitter_count(t.features_id) AS emitter_count,                  -- Number of emitters bound to this feature.
  calc_features_index_builder_count(t.features_id) AS index_builder_count,      -- Number of index builders bound to this feature.
  calc_features_template_count(t.features_id) AS template_count,                -- Number of runtime templates bound to this feature.
  calc_features_table_count(t.features_id) AS table_count,                      -- Number of rulebook tables bound to this feature.
  calc_features_has_bindings(t.features_id) AS has_bindings                     -- TRUE iff at least one FeatureBindings row points at this feature. Distinguishes 'shipped' (has bindings) from 'planned' (no bindings yet) — without a separate state column.
FROM features t;

-- ----------------------------------------------------------------------------
-- vw_feature_bindings: Many-to-many resolution rows linking a Feature to the concrete pieces that implement it: CliCommands, Emitters, IndexBuilders, Templates, rulebook tables, etc. Kind is the discriminator; TargetName names the row inside the bound catalog. Notes is free-form.
-- Combines base table columns with calculated/lookup/aggregation fields.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW vw_feature_bindings WITH (security_invoker = ON) AS
SELECT
  t.feature_bindings_id,                                                        -- Primary key (auto-synthesized: FeatureBindings had no field named 'Id' or ending in 'Id').
  calc_feature_bindings_name(t.feature_bindings_id) AS name,                    -- Compound key: <Feature>::<Kind>::<TargetName>. Auto-derived.
  t.feature,                                                                    -- FK to the Features row.
  t.kind,                                                                       -- One of 'cli', 'emitter', 'index', 'template', 'table'. Tells the consumer which catalog to look TargetName up in.
  t.target_name,                                                                -- Name of the bound row inside its catalog. For example, Kind='cli' / TargetName='sema snapshot prepare'; Kind='emitter' / TargetName='yaml'; Kind='table' / TargetName='SeedRequests'.
  t.notes                                                                       -- Free-form annotation about why this binding exists or any caveat.
FROM feature_bindings t;
