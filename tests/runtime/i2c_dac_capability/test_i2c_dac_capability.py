"""i2c.dac.capability axiom counterexamples — sema must reject what violates an axiom.
...
def test_axiom_1_catches_mux_name_without_mux_channel() -> None:
    payload = json.loads((FIX / "axiom_1.json").read_text())
    with pytest.raises(SemaError, match="MuxPairing"):
