"""i2c.result axiom counterexamples — sema must reject what violates an axiom."""
...
def test_axiom_1_catches_failure_without_error() -> None:
    payload = json.loads((FIX / "axiom_1.json").read_text())
    with pytest.raises(SemaError, match="ErrorIffFailure"):
