"""i2c.dac.writer.component.gt axiom counterexamples — sema must reject what violates an axiom.
...
def test_axiom_1_catches_duplicate_dac_channel() -> None:
    payload = json.loads((FIX / "axiom_1.json").read_text())
    with pytest.raises(SemaError, match="DacChannelUniqueness"):
