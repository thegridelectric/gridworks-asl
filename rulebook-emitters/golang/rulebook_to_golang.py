...
def main() -> int:
    parser = argparse.ArgumentParser(description="Emit Go source from the rulebook.")
    ...
    print(f"rulebook-to-golang: read {args.input}")
    for name, count in table_summary(rulebook):
        print(f"  {name}: {count} rows")

    placeholder = args.output / "TODO.txt"
    placeholder.write_text(
        "rulebook-to-golang: scaffold only. Real emission not yet implemented.\n"
        f"Rulebook: {args.input}\n"
    )
    print(f"rulebook-to-golang: wrote placeholder {placeholder}")
    return 0
