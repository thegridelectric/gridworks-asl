"""
ASL Seed Builder - Generates self-contained asl/ directories for repositories
"""
import shutil
import yaml
...
class SeedBuilder:
    """Generates self-contained ASL directories for repositories."""
    ...
    def generate_seed(
        ...
        """
        Generate a complete asl/ directory.
        ...
        asl_path = output_path / "asl"
        (asl_path / "types").mkdir(parents=True, exist_ok=True)
        (asl_path / "enums").mkdir(parents=True, exist_ok=True)
        
        if include_old_versions:
            (asl_path / "types" / "old_versions").mkdir(exist_ok=True)
        
        if include_tests:
            (asl_path / "tests").mkdir(exist_ok=True)
        
        # Copy selected types and enums
        self._copy_types(asl_path / "types")
        self._copy_enums(asl_path / "enums")
        
        # Copy property_format.py
        self._copy_property_format(asl_path)
        
        # Generate codec.py from template
        self._generate_codec(package_name, repo_name, asl_path)
        
        # Generate __init__.py files
        self._generate_init_files(package_name, asl_path)
        
        # Generate basic tests if requested
        if include_tests:
            self._generate_tests(package_name, asl_path / "tests")
        
        # Generate README
        self._generate_readme(repo_name, asl_path)
    ...
    def _copy_types(self, types_dir: Path):
        ...
        for type_name in self.resolved_types:
            ...
            if source.exists():
                ...
                # Rewrite imports from gwasl.registry to target package
    ...
    def _rewrite_imports(self, content: str, package_name: str) -> str:
        ...
        # Replace imports from gwasl.registry.types
        content = re.sub(
            r'from gwasl\.registry\.types\.(\w+) import',
            fr'from {package_name}.asl.types.\1 import',
            content
        )
        
        # Replace imports from gwasl.registry.enums
        content = re.sub(
            r'from gwasl\.registry\.enums\.(\w+) import',
            fr'from {package_name}.asl.enums.\1 import',
            content
        )
        
        # Replace imports from gwasl.registry (for AslType base class)
        content = re.sub(
            r'from gwasl\.registry import AslType',
            fr'from {package_name}.asl.codec import AslType',
            content
        )
    ...
    def _generate_codec(self, package_name: str, repo_name: str, asl_path: Path):
        ...
        output_file = asl_path / "codec.py"
        output_file.write_text(code)
        print("  Generated codec.py")
    
    def _generate_init_files(self, package_name: str, asl_path: Path):
        """Generate __init__.py files for all directories."""
        
        # Main asl/__init__.py
        main_init = self._generate_main_init(package_name)
        (asl_path / "__init__.py").write_text(main_init)
        
        # types/__init__.py
        types_init = self._generate_types_init(package_name)
        (asl_path / "types" / "__init__.py").write_text(types_init)
        
        # enums/__init__.py
        enums_init = self._generate_enums_init(package_name)
        (asl_path / "enums" / "__init__.py").write_text(enums_init)
        
        # old_versions/__init__.py (if it exists)
        old_versions_dir = asl_path / "types" / "old_versions"
    ...
    def _generate_main_init(self, package_name: str) -> str:
        """Generate the main asl/__init__.py file."""
        ...
        lines = ['"""ASL (Application Shared Language) for this repository."""\n']
        lines.append(f"from {package_name}.asl.codec import AslType, AslCodec, AslError, get_current_types\n")
        
        if type_classes:
            lines.append("\n# Types")
            for type_name, class_name in type_classes:
                module_name = type_name.replace(".", "_")
                lines.append(f"from {package_name}.asl.types.{module_name} import {class_name}")
        
        if enum_classes:
            lines.append("\n# Enums")
            for enum_name, class_name in enum_classes:
                module_name = enum_name.replace(".", "_")
                lines.append(f"from {package_name}.asl.enums.{module_name} import {class_name}")
        
        lines.append("\n__all__ = [")
        lines.append('    # Core')
        lines.append('    "AslType",')
        lines.append('    "AslCodec",')
        lines.append('    "AslError",')
    ...
    def _generate_types_init(self, package_name: str) -> str:
        ...
        lines = ['"""ASL Types for this repository."""\n']
    ...
    def _generate_enums_init(self, package_name: str) -> str:
        ...
        lines = ['"""ASL Enums for this repository."""\n']
    ...
    def _generate_tests(self, package_name: str, tests_dir: Path):
        ...
        test_content = f'''"""Basic tests to verify ASL codec functionality."""

import pytest
from {package_name}.asl import AslCodec, AslType
...
'''
    ...
    def _generate_readme(self, repo_name: str, asl_path: Path):
        """Generate a README for the asl directory."""
        readme_content = f"""# ASL for {repo_name}

This directory contains the ASL (Application Shared Language) types and codec for {repo_name}.
...
```python
from {repo_name.replace('-', '_')}.asl import AslCodec
...
Generated from the GridWorks ASL Registry: https://github.com/thegridelectric/gridworks-asl
"""
        (asl_path / "README.md").write_text(readme_content)
...
if __name__ == "__main__":
    ...
    print(f"\nGenerated ASL directory at: {output / 'asl'}")
