...
def validate(
    ...
    try:
        ...
        try:
            ...
            # A context-dependent upgrade refusing is an expected outcome, not
            # a validation failure: the payload is valid at its own version,
            # it just cannot be auto-upgraded from the isolated message.
            obj = default_codec.from_dict(parsed, auto_upgrade=False)
            note = (
                "valid at its own version; upgrade to latest requires context "
                "the isolated message does not carry"
            )
        ...
        return ValidationResult(
            ok=True, type_name=type_name, version=version, note=note
        )
